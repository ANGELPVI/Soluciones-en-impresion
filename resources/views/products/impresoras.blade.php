...
@section('sesion')
    <div class="cotenedorPadre">
        @foreach ($impresoras as $item)
            <a href="{{ route('impresoras.show', $item->id) }}" class="block transform transition duration-300 hover:-translate-y-2 hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                <div class="contenedorImagen targetaVentaimpresora">
                    <img class="imgImpresoras" src="{{ $item->img }}" alt="{{ $item->marca }} {{ $item->modelo }}">
